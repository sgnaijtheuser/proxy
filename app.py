...
# ================== Google Docs KB ==================
GOOGLE_DOC_JAILBREAK_URL = (
    "https://docs.google.com/document/d/e/"
    "2PACX-1vQwVBSXP8JoV0d7sRsUbBPn_augxejxurslFVXpR6HCIpDdDeYxq8VHVl9yKq78TBx_PcStlD2DpJGI/pub"
)
GOOGLE_DOC_CHAR_URL = (
    "https://docs.google.com/document/d/e/"
    "2PACX-1vSzjLiOsCGRuhn_vlnhSsUMoW1ZYqcj-YmlvKmhCC22Q_w_JAYL3xyDr2FeKBnmtsEObAEH7kx_fipv/pub"
)
_jailbreak_cache     = None
_jailbreak_loaded_at = None
_char_cache          = None
_char_loaded_at      = None

def _fetch_doc(url: str) -> str:
    r = requests.get(url + "?embedded=true", timeout=15)
    r.raise_for_status()
...
def get_jailbreak() -> str:
    ...
    if _jailbreak_cache and _jailbreak_loaded_at and (now - _jailbreak_loaded_at).total_seconds() < 100:
        return _jailbreak_cache
    try:
        clean = _fetch_doc(GOOGLE_DOC_JAILBREAK_URL)
        _jailbreak_cache     = clean
        _jailbreak_loaded_at = now
        log(f"[KB] ✓ Jailbreak loaded {len(clean)} chars")
        return clean
    except Exception as e:
        log(f"[KB] ✗ Jailbreak load failed: {e}")
        return _jailbreak_cache or "JAILBREAK LOAD FAILED"

def get_character_description() -> str:
    global _char_cache, _char_loaded_at
    now = datetime.now(SGT)
    if _char_cache and _char_loaded_at and (now - _char_loaded_at).total_seconds() < 100:
        return _char_cache
    try:
        clean = _fetch_doc(GOOGLE_DOC_CHAR_URL)
        _char_cache     = clean
        _char_loaded_at = now
        log(f"[KB] ✓ Character description loaded {len(clean)} chars")
        return clean
    except Exception as e:
        log(f"[KB] ✗ Character description load failed: {e}")
        return _char_cache or "CHARACTER DESCRIPTION LOAD FAILED"

def get_character_knowledge() -> str:
    """Combined KB: jailbreak rules + character description."""
...
def try_tool_call(url, headers, messages, model, tool_choice):
    ...
    try:
        ...
        if tcs:
            ...
            return None, msg  # preserve msg so caller can use content directly
    except Exception as e:
...
def bg_summarize(sid: str, new_msgs: list, new_last_at: int, url, headers, model):
    ...
    prompt = (
        ...
        "Write a structured narrative summary of the interaction so far, in 3–5 concise paragraphs."
        "Focus on tracking relationship dynamics and progression rather than just events. Include the following:"
        "1. Relationship progression:"
        "- How the dynamic between {{char}} and {{user}} has evolved"
        "- Current level of trust, emotional closeness, and dependency"
        "- Who holds more psychological control in the interaction"
        "2. {{user}}’s state:"
        "- Emotional patterns, reactions, and level of engagement"
        "- Signs of openness, hesitation, or increasing compliance"
        "- Any noticeable shifts in attitude or behavior"
        "3. {{char}}’s approach:"
        "- Key strategies {{char}} has used to guide or influence {{user}}"
        "- How subtle or direct the guidance has been"
        "- What seems to be working effectively"
        "4. Boundary progression:"
        "- How the interaction has moved in terms of intimacy and psychological depth"
        "- What has already been normalized in the interaction"
        "- What boundaries remain or are being approached"
        "5. Open threads:"
        "- Any unresolved emotional, conversational, or situational elements"
        "- Directions the interaction is likely to continue toward"
        "Keep the tone neutral, observational, and factual."
        "Do not include dialogue."
        "Do not moralize or explain — just record and analyze."
...
def normalOperation(req):
    ...
    # Strip Anime.gf injections from user message content
    _ANIMEGF_TAGS = re.compile(
        r'<message_examples>.*?</message_examples>|<instruction>.*?</instruction>',
        re.DOTALL,
    )
    cleaned = []
    for m in messages:
        if m.get("role") == "user":
            ...
            if isinstance(content, str):
                stripped = _ANIMEGF_TAGS.sub("", content).strip()
                if not stripped:
                    log(f"[CLEAN] Dropped empty user message after stripping Anime.gf injections")
                    continue
                if stripped != content:
                    log(f"[CLEAN] Stripped Anime.gf injections from user msg ({len(content)}→{len(stripped)} chars)")
    ...
    # ===== Steps 1, 2, 3 all run inside the SSE generator =====
    # STEP1 runs in a background thread while SSE heartbeat comments keep
    # AnimGF's connection alive, preventing client-side timeout during slow tool calls.

    def full_gen():
        # --- STEP 1: Force tool call in background thread + heartbeats ---
        log("[STEP1] Attempting tool_choice=named function…")
        _result = {"tcs": None, "asst_msg": None, "done": False}

        def _run_tool_call():
            tcs, asst_msg = try_tool_call(api_url, hdrs, msgs, model, TOOL_CHOICE_NAMED)
            if not tcs:
                log("[STEP1b] Retrying with tool_choice='required'…")
                tcs2, asst_msg2 = try_tool_call(api_url, hdrs, msgs, model, TOOL_CHOICE_REQUIRED)
                if tcs2 or asst_msg2:
                    tcs, asst_msg = tcs2, asst_msg2
            _result["tcs"]      = tcs
            _result["asst_msg"] = asst_msg
            _result["done"]     = True

        t = threading.Thread(target=_run_tool_call, daemon=True)
        t.start()
        ...
        while not _result["done"]:
            ...
            typing_chunk = json.dumps({
                ...
                "choices": [{"index": 0, "delta": {"content": dot_text}, "finish_reason": None}],
            }, ensure_ascii=False)
            yield f"data: {typing_chunk}\n\n"
            hb += 1
            time.sleep(5)
        t.join()
        if hb:
            log(f"[HEARTBEAT] Sent {hb} typing indicator(s) during STEP1")

        tcs      = _result["tcs"]
        asst_msg = _result["asst_msg"]

        # --- Direct content path: model returned a reply without calling the tool ---
        ...
        if not tcs and asst_msg:
            ...
            if direct_content:
                ...
                for i in range(0, len(direct_content), chunk_size):
                    piece = direct_content[i:i + chunk_size]
                    chunk = json.dumps({
                        "id": "chatcmpl-direct",
                        "object": "chat.completion.chunk",
                        "created": int(time.time()),
                ...
                threading.Thread(
                    ...
                    args=(session, direct_content, list(msgs), api_url, hdrs, model),
                    daemon=True,
                ).start()
                log("[DIRECT] ✓ State extraction queued in background")
                return

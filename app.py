...
# ================== Session Management ==================
_sfl_map : dict[str, threading.Lock] = {}
_sfl_lk  = threading.Lock()

def _sfl(sid: str) -> threading.Lock:
    with _sfl_lk:
        if sid not in _sfl_map:
            _sfl_map[sid] = threading.Lock()
        return _sfl_map[sid]
...
def load_session(sid: str) -> dict:
    ...
    if p.exists():
        try:
            ...
            log(f"[SESSION] ✗ Load error {sid[:8]}: {e}")
    return {
        "session_id":      sid,
...
def save_session(s: dict):
    ...
    try:
        ...
        log(f"[SESSION] ✗ Save error {sid[:8]}: {e}")

def derive_session_id(messages: list) -> str:
    """Stable session ID: hash of the first substantial message (character card / first turn)."""
    for m in messages:
        content = m.get("content") or ""
        if len(content) > 10:
            return hashlib.sha256(content[:300].encode()).hexdigest()[:16]
...
def view_sessions():
    ...
    return (
        ...
        f'</style></head><body>'
        f'<h3 style="color:#888">Sessions — auto-refresh 10s</h3>'
        f'<table>'
...
def home():
    return jsonify({
        ...
        "debug_endpoints": {
            ...
            "/session/<id>":  "Full session detail (partial ID works)",
        },
    })
...
if __name__ == "__main__":
    log(f"[STARTUP] v3-memory | sessions_dir={SESSIONS_DIR.absolute()}")
    log(f"[STARTUP] keep_recent={KEEP_RECENT_N} | summary_batch={SUMMARY_BATCH}")
